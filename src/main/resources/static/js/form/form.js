...
const addQuestion = async (event) =>{
    ...
    let type = tableContainer.classList[1];
...
function addQuestionInput(event) {
    const tableContainer = event.target.parentNode.parentNode;
    const rows = tableContainer.querySelector('.rows');
    const button = tableContainer.querySelector('.create');
    console.log(rows)
    console.log(tableContainer)
    if (tableContainer.classList.contains('textInput') || tableContainer.classList.contains('numericInput')) {
        rows.innerHTML += `
    <div class="row">
